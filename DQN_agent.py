...
from network_tf2 import Network
from replay import Replay
from config import Network_parameters, Training_parameters
...
class DQN_Agent:
    ...
    def __init__(self, parameters):
        ...
        self.Q_net = Network(self.state_dim, self.action_dim, Network_parameters.Q_net_var_scope)
        self.target_Q_net = Network(self.state_dim, self.action_dim, Network_parameters.target_Q_net_var_scope)
    ...
    def train(self):
        ...
        if self.render_decision:
            ...
            self.env.render()
            video_save_path = f'Video_DQN_{self.env_name}/'
        ...
        for episode in range(self.train_episodes):
            ...
            while not done:
                ...
                # epsilon = 0.05 - episode * ((0.05 - 0.005) / self.train_episodes)
                epsilon = 0.05
                ...
                targets = batch_reward + np.where(check_if_not_terminal, self.discount * np.max(target_Q_next, axis=1),
                                                  0)
                actions_selected = np.vstack([np.arange(Network_parameters.batch_size), batch_action]).T
                self.Q_net.fit(batch_state, targets, actions_selected, episode)
...
def parse_arguments():
    ...
    # parser.add_argument('--e', dest='environment_name', default='MountainCar-v0', type=str,
